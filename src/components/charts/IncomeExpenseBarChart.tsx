      ...
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }} barGap={4}>
          ...
          <Tooltip
            ...
            formatter={(value: any) => [`${symbol}${Number(value).toLocaleString()}`, undefined]}
            labelStyle={tooltipLabelStyle}
          />
          <Legend iconType="square" iconSize={10} wrapperStyle={legendStyle} />
          <Bar dataKey="income" name="Income" fill="var(--accent)" radius={[3, 3, 0, 0]} barSize={18} />
          <Bar dataKey="expenses" name="Expenses" fill="var(--text-secondary)" radius={[3, 3, 0, 0]} barSize={18} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
